...
int main() {
    ...
    // 打开视频文件
    bool success = processor.setInput("../workspace/scene.mp4"); 
    // // 声明显示视频的窗口
    ...
    // 设置输出为视频，如果是保存成视频就不能使用canny作为回调函数了，canny只接受灰度图
    // processor.setOutput("../workspace/Out.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'));
    // 设置输出为图像
    processor.setOutput("../workspace/", ".png");
    // 开始处理
    processor.run();
    return 0;
}