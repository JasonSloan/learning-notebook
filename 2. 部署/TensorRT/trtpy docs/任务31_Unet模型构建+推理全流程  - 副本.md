...
void inference(){
    ...
    // 做逆仿射变换的时候，最大概率值需要使用双线性插值
    cv::warpAffine(prob, prob, m2x3_d2i, image.size(), cv::INTER_LINEAR);
    // 做逆仿射变换的时候，类别索引需要使用最近邻插值（如果使用双线性插值，那么就会出现小数的类别索引）
