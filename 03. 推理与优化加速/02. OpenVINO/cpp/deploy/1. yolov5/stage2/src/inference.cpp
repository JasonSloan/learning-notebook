...
public:
    ...
    void worker(promise<bool>& pro){
        ...
        // load success
        pro.set_value(true);  
        printf("Model loaded successfully: %s\n", modelPath.c_str());                                                  // 将start_up中pro.get_future().get()的值设置为true
    ...
    // 实际上forward函数是生产者
    virtual vector<Result> forward(std::vector<cv::Mat> input_images, 
        ...
        vector<Result> result = fut.get();
        return result;                                                          // 等待模型将推理数据返回fut，然后fut再将数据return出去
    }
