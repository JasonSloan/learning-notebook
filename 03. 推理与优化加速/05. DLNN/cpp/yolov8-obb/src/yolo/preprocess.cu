...
__global__ void warp_affine_bilinear_kernel(
    ...
    // __syncthreads();  // if you dont use shared memory, dont use this function
}

__global__ void transpose_and_normalize_kernel(
    uint8_t* src_device, float* dst_device,
    int width, int height, int feat_size, size_t offset
){
    int dx = blockDim.x * blockIdx.x + threadIdx.x; 
    int dy = blockDim.y * blockIdx.y + threadIdx.y;
    if (dx >= width || dy >= height)  return;
    
    int pix_idx = dy * width + dx;

    float* dst_device_r = dst_device + offset + 0 * feat_size + pix_idx;        
    float* dst_device_g = dst_device + offset + 1 * feat_size + pix_idx;
    float* dst_device_b = dst_device + offset + 2 * feat_size + pix_idx;

    // float* dst_device_r = dst_device + 0 * feat_size + pix_idx;        
    // float* dst_device_g = dst_device + 1 * feat_size + pix_idx;
    // float* dst_device_b = dst_device + 2 * feat_size + pix_idx;

    *dst_device_r = src_device[pix_idx * 3 + 0] / 255.0f;
    *dst_device_g = src_device[pix_idx * 3 + 1] / 255.0f;
    *dst_device_b = src_device[pix_idx * 3 + 2] / 255.0f;
...
void preprocess_kernel_invoker(
    ...
    warp_affine_bilinear_kernel<<<grid_size, block_size, 0, nullptr>>>(
        src_device, src_line_size, src_width, src_height,
        intermediate_device, dst_line_size, dst_width, dst_height,
        fill_value, affine
    );

    transpose_and_normalize_kernel<<<grid_size, block_size, 0, nullptr>>>(
        intermediate_device, dst_device, dst_width, dst_height, dst_img_area, offset
    );
}