...
public:
    ...
    void inference(Job& job){
            ...
            // ===============================后处理================================    
            // 这里没用智能指针，智能指针比原生指针慢10多ms
            auto output_data_uchar = new uint8_t[output_numel];
            for (int i = 0; i < output_numel; ++i){
                // 这里用的static_cast没用cv::saturate_cast，因为static_cast比cv::saturate_cast快30ms
                output_data_uchar[i] = static_cast<uint8_t>(output_data_host[i]);
            }

            // 将output_data_uchar转化成py::array_t<uint8_t>类型
            py::array_t<uint8_t> output_array(output_numel, output_data_uchar);
            // 将array的shape设置成output_height, output_width, output_channel，然后在flask的代码中就可以直接用cv2.imdecode()解码成cv::Mat
            output_array.resize({output_height, output_width, output_channel});
