...
1，一个工程中只能有一个main()函数
2，头文件（.h）中不要使用using namespace ，因为如果使用了，当你在导入这个头文件的时候，就会导入头文件中使用的using namespace的命名空间，造成混乱。
3，在编译时，有main函数的文件会被编译成可执行文件，无main函数的文件会被编译成动态库。

4，nullptr和void指针辨析:nullptr需要指定指针类型(int float等)，指针为空指针，使用if判断时，nullptr为false;void为未指定类型的指针，可以先初始化，然后再指向任意类型数据的地址。
例子:
...
静态连编与动态连编:静态连编通过指针或者引用的类型来确定调用的函数，动态连编通过对象的类型确定调用函数。
```
...
枚举类型：(变量的值只能在列举出来的值的范围内)  --> python中的assert
...
该方法把参数的引用赋值给形式参数。在函数内，该引用用于访问调用中要用到的实际参数。这意味着，修改形式参数会影响实际参数。
 ...
 四、函数重载（一般会避免函数同名）
 在同一个作用域内，可以声明几个功能类似的同名函数，但是这些同名函数的形式参数（指参数的个数、类型或者顺序）  必须不同。您不能仅通过返回类型的不同来重载函数。python中也会重载函数，但是一般不用。
 class A:
    def get_name(self, b:float):
        return b
    def get_name(self, c:int):
        return c
a = A()
print(a.get_name(1.2))  # 返回1.2
print(a.get_name(12))   # 返回12
函数重载满足条件：
同一个作用域下
函数名称相同
...
```C++
栈区：在函数内部声明的所有变量都将占用栈内存
堆区：这是程序中未使用的内存，在程序运行时可用于动态分配内存
...
七、类的公有静态成员变量
类的公有静态成员变量意义在于，类及类的多个实例对象可以访问该变量修改该变量，注意该变量需在类的外部定义初始化，否则如果在类的声明中初始化，就会必须为const类型，就没啥意义了。
