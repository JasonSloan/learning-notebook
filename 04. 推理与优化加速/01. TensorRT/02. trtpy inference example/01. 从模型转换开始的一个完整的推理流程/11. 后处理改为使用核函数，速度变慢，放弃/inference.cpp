...
public:
    ...
    void inference(Job& job){
            ...
            /*----------------------------后处理计时开始---------------------------------*/
            float* src_host = output_data_host;
            float* src_device = nullptr;
            checkRuntime(cudaMalloc(&src_device, output_numel * sizeof(float)));
            checkRuntime(cudaMemcpyAsync(src_device, src_host, output_numel * sizeof(float), cudaMemcpyHostToDevice, stream));
            uint8_t* dst_device = nullptr;
            uint8_t* dst_host = nullptr;
            checkRuntime(cudaMalloc(&dst_device, output_numel));
            checkRuntime(cudaMallocHost(&dst_host, output_numel));
            int src_line_size = output_width * output_channel;
            int dst_line_size = output_width * output_channel;
            data_cast(src_device, dst_device, output_width, output_height, src_line_size, dst_line_size);
            checkRuntime(cudaMemcpyAsync(dst_host, dst_device, output_numel, cudaMemcpyDeviceToHost, stream));
            checkRuntime(cudaStreamSynchronize(stream));
            std::vector<uint8_t> output_vector(dst_host, dst_host + output_numel);
            checkRuntime(cudaFreeHost(dst_host));
